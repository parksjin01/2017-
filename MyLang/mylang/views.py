...
class Downloader(threading.Thread):
	...
	def run(self):
		...
		if 'http' not in url:
			...
			user.message_box = cPickle.loads(str(message_box))
			user.save()
...
def mypage(request):
	...
	for message in message_boxs:
		message_box.append([time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(message.date)), message.content, message.new])
		tmp = coopy.deepcopy(message)
		tmp.new = 0
...
def vocabulary(request):
	...
	with open(str('/home/knight/MyLang/mylang/number.txt'), 'rt') as f:
		num = int(f.read())
	try:
            cur_user = request.COOKIES.get('ec')
        except:
	...
	for idx in test:
		...
		tmp_meaning = pickle.loads(voca.objects.get(id=idx+3335).korean)[random.randrange(len(pickle.loads(voca.objects.get(id=idx+3335).korean)))]
		meaning.append(tmp_meaning)
		while len(meaning) < example:
			rand = random.randrange(num)
			tmp_meaning = pickle.loads(voca.objects.get(id=rand+3335).korean)[random.randrange(len(pickle.loads(voca.objects.get(id=rand+3335).korean)))]
